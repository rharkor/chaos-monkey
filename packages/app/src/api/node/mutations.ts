...
export const addNode = async ({ input, ctx: { session } }: apiInputFromSchema<typeof addNodeSchema>) => {
  ensureLoggedIn(session)
  try {
    const { name, ip } = input
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2002") {
        const meta = error.meta
        if (!meta) ApiError("node.exist")
        if ((meta.target as Array<string>).includes("name")) {
          return ApiError("node.exist")
        }
      }
    }
    return handleApiError(error)
...
export const deleteNode = async ({ input, ctx: { session } }: apiInputFromSchema<typeof deleteNodeSchema>) => {
  ensureLoggedIn(session)
  try {
    const { id } = input
...
export const updateNode = async ({ input, ctx: { session } }: apiInputFromSchema<typeof updateNodeSchema>) => {
  ensureLoggedIn(session)
  try {
    const { id, name, ip } = input
